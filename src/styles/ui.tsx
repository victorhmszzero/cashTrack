...
export const Flex = styled.div<{
  ...
  display: flex;
  flex-direction: ${p => p.$col ? 'column' : 'row'***REMOVED***
  gap: ${p => p.$gap !== undefined ? `${p.$gap * 0.25}rem` : '0'***REMOVED***
  align-items: ${p => p.$align || 'stretch'***REMOVED***
  justify-content: ${p => p.$justify || 'flex-start'***REMOVED***
  flex-wrap: ${p => p.$wrap ? 'wrap' : 'nowrap'***REMOVED***
  flex-shrink: ${p => p.$shrink0 ? '0' : '1'***REMOVED***
`
...
export const Divider = styled.div<{ $vertical?: boolean }>`
  ${p =>
    p.$vertical
      ? css`width: 1px; align-self: stretch; background: ${p.theme.border***REMOVED*** flex-shrink: 0;`
      : css`height: 1px; width: 100%; background: ${p.theme.border***REMOVED***`}
`
...
export const Card = styled.div`
  ...
  border-radius: ${p => p.theme.radius.lg***REMOVED***
  box-shadow: ${p => p.theme.shadow***REMOVED***
  overflow: hidden;
`
...
export const SectionTitle = styled.h3`
  font-size: 0.875rem;
  font-weight: 600;
  color: ${p => p.theme.text.secondary***REMOVED***
  margin: 0;
`

export const Muted = styled.span<{ $size?: 'xs' | 'sm' }>`
  color: ${p => p.theme.text.muted***REMOVED***
  font-size: ${p => p.$size === 'xs' ? '0.6875rem' : '0.75rem'***REMOVED***
`
...
export const PrimaryButton = styled.button`
  ${buttonBase}
  padding: 0.5rem 1rem;
  background: ${p => p.theme.accent***REMOVED***
  color: white;
...
export const IconButton = styled.button<{ $danger?: boolean }>`
  ...
  background: transparent;
  color: ${p => p.$danger ? p.theme.danger : p.theme.text.muted***REMOVED***
  transition: all 0.15s;
  font-family: inherit;

  &:hover {
    background: ${p => p.$danger ? p.theme.danger + '18' : p.theme.bg.hover***REMOVED***
    color: ${p => p.$danger ? p.theme.danger : p.theme.text.primary***REMOVED***
  }
`
...
const inputBase = css`
  ...
  &::placeholder { color: ${p => p.theme.text.faint***REMOVED*** }

  &:focus {
    outline: none;
    border-color: ${p => p.theme.accent***REMOVED***
    box-shadow: 0 0 0 3px ${p => p.theme.accentSoft***REMOVED***
  }
...
export const Th = styled.th<{ $align?: string; $clickable?: boolean; $sticky?: boolean }>`
  text-align: ${p => p.$align || 'left'***REMOVED***
  ...
  color: ${p => p.theme.text.muted***REMOVED***
  border-bottom: 1px solid ${p => p.theme.border***REMOVED***
  cursor: ${p => p.$clickable ? 'pointer' : 'default'***REMOVED***
  background: ${p => p.theme.bg.card***REMOVED***
  ...
  ${p => p.$clickable && css`
    user-select: none;
    &:hover { color: ${p.theme.text.primary***REMOVED*** }
  `}
`
...
export const Td = styled.td<{
  ...
  text-align: ${p => p.$align || 'left'***REMOVED***
  padding: 0.625rem 1rem;
  color: ${p => p.$muted ? p.theme.text.muted : p.theme.text.primary***REMOVED***
  border-bottom: 1px solid ${p => p.theme.border}55;
  font-size: 0.875rem;
  font-family: ${p => p.$mono ? p.theme.font.mono : 'inherit'***REMOVED***

  ${p => p.$sticky && css`
    position: sticky;
    left: 0;
    z-index: 5;
    background: ${p.theme.bg.card***REMOVED***
  `}
`
...
export const Tr = styled.tr<{
  ...
  opacity: ${p => p.$faded ? 0.38 : 1***REMOVED***
  display: ${p => p.$faded ? 'none' : 'solid'***REMOVED***
  cursor: ${p => p.$clickable ? 'pointer' : 'default'***REMOVED***
...
export const Badge = styled.span<{ $color?: string }>`
  ...
  border-radius: 6px;
  background: ${p => p.$color ? p.$color + '18' : p.theme.bg.subtle***REMOVED***
  color: ${p => p.$color || p.theme.text.muted***REMOVED***
`
...
export const ProgressFill = styled.div<{ $pct: number; $color?: string }>`
  height: 100%;
  width: ${p => p.$pct}%;
  background: ${p => p.$color || p.theme.accent***REMOVED***
  border-radius: 99px;
...
export const ColorSwatch = styled.button<{ $color: string; $active: boolean }>`
  ...
  height: 2.25rem;
  border-radius: 50%;
  border: 3px solid ${p => p.$active ? 'white' : 'transparent'***REMOVED***
  outline: ${p => p.$active ? `2px solid ${p.$color}` : 'none'***REMOVED***
  background: ${p => p.$color***REMOVED***
  cursor: pointer;
