...
import { AppState, CreditCard, Purchase, FixedBill, PixPerson, PixItem, AppSettings, Category } from '../types'
...
interface Store extends AppState {
  ...
  setPurchaseOverride: (cardId: string, purchaseId: string, month: string, amount: number | null) => void
  ...
  setFixedBillOverride: (id: string, month: string, amount: number | null) => void
...
export const useStore = create<Store>()(
  persist(
    (set) => ({
      ...
      addCard: card => set(state => ({ cards: [...state.cards, { ...card, id: uuid(), purchases: [] }] })),
      updateCard: (id, data) => set(state => ({ cards: state.cards.map(c => c.id === id ? { ...c, ...data } : c) })),
      deleteCard: id => set(state => ({ cards: state.cards.filter(c => c.id !== id) })),

      addPurchase: (cardId, p) => set(state => ({
        cards: state.cards.map(c => c.id === cardId ? { ...c, purchases: [...c.purchases, { ...p, id: uuid() }] } : c),
      })),
      updatePurchase: (cardId, purchaseId, data) => set(state => ({
        cards: state.cards.map(c => c.id === cardId
          ? { ...c, purchases: c.purchases.map(p => p.id === purchaseId ? { ...p, ...data } : p) }
          : c),
      })),
      deletePurchase: (cardId, purchaseId) => set(state => ({
        cards: state.cards.map(c => c.id === cardId
          ? { ...c, purchases: c.purchases.filter(p => p.id !== purchaseId) }
          : c),
      })),
      setPurchaseOverride: (cardId, purchaseId, month, amount) => set(state => ({
        cards: state.cards.map(c => c.id === cardId ? {
          ...c,
      ...
      addFixedBill: bill => set(state => ({ fixedBills: [...state.fixedBills, { ...bill, id: uuid() }] })),
      updateFixedBill: (id, data) => set(state => ({ fixedBills: state.fixedBills.map(b => b.id === id ? { ...b, ...data } : b) })),
      deleteFixedBill: id => set(state => ({ fixedBills: state.fixedBills.filter(b => b.id !== id) })),
      setFixedBillOverride: (id, month, amount) => set(state => ({
      ...
      addPixPerson: name => set(state => ({ pixPeople: [...state.pixPeople, { id: uuid(), name, items: [] }] })),
      updatePixPerson: (id, name) => set(state => ({ pixPeople: state.pixPeople.map(p => p.id === id ? { ...p, name } : p) })),
      deletePixPerson: id => set(state => ({ pixPeople: state.pixPeople.filter(p => p.id !== id) })),
      addPixItem: (personId, item) => set(state => ({
        pixPeople: state.pixPeople.map(p => p.id === personId ? { ...p, items: [...p.items, { ...item, id: uuid() }] } : p),
      })),
      updatePixItem: (personId, itemId, data) => set(state => ({
        pixPeople: state.pixPeople.map(p => p.id === personId
          ? { ...p, items: p.items.map(i => i.id === itemId ? { ...i, ...data } : i) }
          : p),
      })),
      deletePixItem: (personId, itemId) => set(state => ({
        pixPeople: state.pixPeople.map(p => p.id === personId
          ? { ...p, items: p.items.filter(i => i.id !== itemId) }
          : p),
      })),

      addCategory: cat => set(state => ({ categories: [...(state.categories ?? []), { ...cat, id: uuid() }] })),
