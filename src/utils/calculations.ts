...
export function isActiveInMonth(
	...
	return (
		startMonth <= yearMonth && (endMonth === null || endMonth >= yearMonth)
	);
}
...
export function formatMonthLabel(yearMonth: string): string {
	...
	return format(date, "MMM'/'yy", { locale: ptBR }).replace(/^\w/, (c) =>
		c.toUpperCase(),
	);
}
...
export function computeSummaries(state: AppState): MonthSummary[] {
	...
	return months.map((ym) => {
		...
		// PIX breakdown
		const pixBreakdown = pixPeople
			.map((person) => {
				const itemsTotal = sumActiveItems(person.items, ym);
				const purchasesTotal = cards.reduce(
					(cs, card) =>
						cs +
						card.purchases
							.filter(
								(p) =>
									(p.paidBy === person.id || p.paidBy === person.name) &&
									isActiveInMonth(p.startMonth, p.endMonth, ym),
							)
				...
				return {
					...
					total: itemsTotal + purchasesTotal,
				};
			})
			.filter((p) => p.total > 0)
		...
		const toLazer =
			balance > 0 ? balance * ((100 - settings.investPercent) / 100) : 0;
